...
# df['DEBIT (£)'] = '{0:.2f}'.format(float(df['DEBIT (£)']))
...
# _________________________________CREATING A HIGH-LEVEL "TK" OBJECT - ACTS AS THE MAIN WINDOW FOR THE APPLICATION__________________________________
...
# _____________ CALLABLE FUNCTIONS, LINKS TO THE "BUTTON" WIDGET, DRIVES THE GUI'S "ACCOUNTING PERIOD", ON THE DISPLAY WINDOW________________
...
def row_picker():
    ...
    for index, date in txn_dates.items():
        ...
        # print(df)
        # class 'numpy.float64'>
        # print(data_dict['DEBIT (£)'][index])
        # print(data_dict)
    # print(booleans)
    # print(new_debit_amount, type(new_debit_amount), "=",
    #       txn_debits[index], "+", float(debit_entry_variable.get()))
    # print("txn_debits[index]: ", txn_debits[index])
    # print("index: ", index, type(index), "date: ", date,
    #       "desired_date: ", desired_date)
    # print(txn_debits)
    # print("(A)", "txn_debits[index=0]:", txn_debits[index], "type: ", type(
    #     txn_debits[index]))
    # print("(B)", "debit_entry_variable.get(): ", debit_entry_variable.get(
    # ), "type: ", type(debit_entry_variable.get()))
    # print("(C)", "(A)+(B): ", txn_debits[index] + int(float(debit_entry_variable.get())), type(
    #     txn_debits[index] + int(float(debit_entry_variable.get()))))
...
# __________________( 1ofXXTEMPT TURN OFF 1825 10062020) UPDATING TRANSACTIONS (DEBIT & CREDIT), VIA DATE ENTRY________________________
# ****
